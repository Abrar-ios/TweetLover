                    ...
                    havingVersion:(NSString*)iconVersionString
{
    ...
    NSString *folderPath = [documentsPath
                            stringByAppendingPathComponent:@"UserProfileImages"];
...
- (UIImage *)cachedImage
{
    NSString * path = [NSString stringWithFormat:@"%@",self.URL];
    path = [path stringByReplacingOccurrencesOfString:@"/" withString:@"-"];
    UIImage * resultImage=[self retrieveImageFromPhone:path havingVersion:nil];
    if (resultImage!=nil)
    {
        return resultImage;
    }
    
    // it's line of code required or not
	...
	if (!self.cancelled)
	{
        if (image && self.URL)
        {
            ...
            if (storeInCache)
            {
                ...
                NSString * strPath = [[self createdirectoryInDocument] stringByAppendingPathComponent:path];
                NSArray* foo = [filename componentsSeparatedByString: @"."];
                NSString* extension = [foo lastObject];
                //   //NSLog(@"file name :%@ path:%@ image:%@",[self.URL lastPathComponent],strPath,image);
                ...
                else if ([[extension lowercaseString] isEqualToString:@"jpg"] || [[extension lowercaseString] isEqualToString:@"jpeg"])
                {
                    ...
                    //                                      @"test" ];
                    if ([UIImageJPEGRepresentation(image, 1.0) writeToFile:strPath atomically:YES])
                    {
                        //NSLog(@"saved");
                    }
                    else
		...
		NSMutableDictionary *userInfo = [NSMutableDictionary dictionaryWithObjectsAndKeys:
									 ...
									 nil];
        //It's if condition is required or not
		if (self.cache)
		{
...
/*
 *directory creating in documnet directory....
 */
-(NSString*)createdirectoryInDocument
{
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    NSString *documentsDirectory = [paths objectAtIndex:0]; // Get documents folder
    NSString *dataPath = [documentsDirectory stringByAppendingPathComponent:@"/GreatControversyAssets"];
